...
std::pmr::string removePolymorphicType(const ObjectGraphBuilder& builder,
...
std::pmr::string removeVertex(const ObjectGraphBuilder& builder,
    const Graph& s, std::string_view name,
    std::pmr::memory_resource* scratch) {
    return {};
    pmr_ostringstream oss(std::ios::out, scratch);
...
std::pmr::string ObjectGraphBuilder::generateAddEdge(bool property,
...
std::pmr::string ObjectGraphBuilder::generateRemoveEdges(bool bInline) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
...
std::pmr::string ObjectGraphBuilder::generateRemoveEdge(bool bInline) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
...
std::pmr::string ObjectGraphBuilder::generateRemoveEdgeIter(bool bInline) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
...
std::pmr::string ObjectGraphBuilder::addVertexTemplate(bool full, bool piecewise) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
    ...
    if (full) {
        if (s.hasVertexProperty() || !s.mComponents.empty() || s.isPolymorphic()) {
            int count = 0;
            OSS << "template <";
            if (s.isPolymorphic()) {
                if (piecewise) {
                    oss << (count++ == 0 ? "" : ", ") << "class Tag";
                    oss << (count++ == 0 ? "" : ", ") << "class TypeT";
                } else {
                    oss << (count++ == 0 ? "" : ", ") << "class Type";
                }
            }
            if (s.hasVertexProperty()) {
                oss << (count++ == 0 ? "" : ", ") << "class VertexProp";
                if (piecewise) {
                    oss << "T";
                }
            }
            for (const auto& c : s.mComponents) {
                oss << (count++ == 0 ? "" : ", ") << "class " << removeTail(c.mName);
                if (piecewise) {
                    oss << "T";
                }
            }
...
std::pmr::string ObjectGraphBuilder::addVertexFunction(bool full, bool piecewise, bool hasDefault) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
    ...
    if (hasDefault) {
        name = cpp.getDependentName(cpp.mCurrentPath);
    } else {
        name = cpp.getImplName();
    }
    ...
    if (full) {
        ...
        if (piecewise) {
            if (s.isPolymorphic()) {
                ...
                oss << (count++ == 0 ? "" : ", ") << "std::piecewise_construct_t";
            }
        }
        if (s.isPolymorphic()) {
            if (piecewise) {
                oss << (count++ == 0 ? "" : ", ") << "TypeT&& valT";
            } else {
                oss << (count++ == 0 ? "" : ", ") << "Type&& val";
            }
        }
        if (s.hasVertexProperty()) {
            if (piecewise) {
                oss << (count++ == 0 ? "" : ", ") << "VertexPropT&& vpT";
            } else {
                oss << (count++ == 0 ? "" : ", ") << "VertexProp&& vp";
            }
        }
        for (const auto& c : s.mComponents) {
            oss << (count++ == 0 ? "" : ", ");
            if (c.mCounterName.empty()) {
                ...
                if (piecewise) {
                    oss << "std::span<" << removeTail(c.mName) << "T> "
                        << getTagName(c.mName, scratch);
                } else {
                    oss << "std::span<" << removeTail(c.mName) << "> "
                        << getTagName(c.mName, scratch);
                }
            }
        }
        if (s.mReferenceGraph) {
            if (hasDefault) {
                oss << (count++ == 0 ? "" : ", ") << "Vertex u = null_vertex()";
            } else {
                oss << (count++ == 0 ? "" : ", ") << name << "::Vertex u";
            }
        }
    } else {
...
std::pmr::string ObjectGraphBuilder::addVertexVectorImpl(bool full, bool piecewise) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
    ...
    std::pmr::set<std::pmr::string> params(scratch);

    if (piecewise) {
        if (s.isVector()) {
            if (s.mReferenceGraph && s.mUniqueName) {
                params.emplace(s.mNamedConcept.mComponentName);
            }
            for (const auto& map : s.mVertexMaps) {
                Expects(map.isBimap());
                params.emplace(map.mComponentName);
            }
        } else {
            for (const auto& c : s.mComponents) {
                params.emplace(c.mName);
            }
        }
        if (!params.empty()) {
            OSS << "// ComponentGraph pre-construction\n";
        }
        for (const auto& c : s.mComponents) {
            if (!params.contains(c.mName)) {
                continue;
            }
            auto tagName = removeTail(c.mName);
            auto paramName = getTagName(c.mName, scratch);
            const auto& valueID = locate(c.mValuePath, g);
            const auto& valueType = g.getDependentCppName(ns, valueID);
            if (g.isPmr(valueID)) {
                OSS << "auto " << paramName << " = std::apply(\n";
                OSS << "    [&](auto&&... args) {\n";
                OSS << "        return " << valueType << "(std::forward<decltype(args)>(args)..., get_allocator());\n";
                OSS << "    },\n";
                OSS << "    std::forward<" << tagName << "T>(" << paramName << "T));\n";
            } else {
                OSS << "auto " << paramName << " = std::make_from_tuple<" << valueType << ">(\n";
                OSS << "    std::forward<" << tagName << "T>(" << paramName << "T));\n";
            }
        }
    }

    if (s.mAddressable && s.mNamed && s.mUniqueName) {
        Expects(s.mReferenceGraph);
        OSS << "// AddressableGraph pre-condition\n";
        auto paramName = getTagName(s.mNamedConcept.mComponentName, scratch);
        if (false) {
            OSS << "Expects(u != null_vertex() || uniqueAddress());\n";
        }
        OSS << "Expects(find_child(u, " << paramName << ") == null_vertex());\n";
    }

    OSS << "// Graph\n";

    auto outputAddVertex = [&]() {
        if (s.mIncidence) {
            if (piecewise) {
                if (s.hasVertexProperty()) {
                    OSS << "std::apply(\n";
                    INDENT();
                    OSS << "[&](auto&&... args) {\n";
                    OSS << "    mVertices.emplace_back(std::forward<decltype(args)>(args)...);\n";
                    OSS << "},\n";
                    OSS << "std::forward<VertexPropT>(vpT));\n";
                } else {
                    OSS << "mVertices.emplace_back();\n";
                }
            } else {
                OSS << "mVertices.emplace_back(";
                if (s.hasVertexProperty()) {
                    oss << "std::forward<VertexProp>(p)";
                }
                oss << ");\n";
            }
        }
        if (s.mGarbageCollection) {
            OSS << "mRootBitset.emplace_back(false);\n";
        }
    };
    auto outputSetVertex = [&]() {
        if (piecewise) {
            if (s.hasVertexProperty()) {
                OSS << "std::apply(\n";
                INDENT();
                OSS << "[&](auto&&... args) {\n";
                if (bPmr) {
                    OSS << "    mVertices[v] = VertexType(std::forward<decltype(args)>(args)..., get_allocator());\n";
                } else {
                    OSS << "    mVertices[v] = VertexType(std::forward<decltype(args)>(args)...);\n";
                }
                OSS << "},\n";
                OSS << "std::forward<VertexPropT>(vpT));\n";
            } else {
                if (bPmr) {
                    OSS << "mVertices[v] = VertexType(get_allocator());\n";
                } else {
                    OSS << "mVertices[v] = VertexType{};\n";
                }
            }
        } else {
            OSS << "mVertices[v] = VertexType(";
    ...
    if (s.isVector()) {
        if (s.mRecycle) {
            OSS << "bool bAdded;\n";
            OSS << "Vertex v;\n";
            OSS << "auto iter = std::find(mEmptyBitset.begin(), mEmptyBitset.end(), true);\n";
            OSS << "if (iter == mEmptyBitset.end()) {\n";
            {
                INDENT();
                OSS << "bAdded = true;\n";
                OSS << "v = static_cast<Vertex>(num_vertices());\n";
                OSS << "mEmptyBitset.emplace_back(false);\n";
                outputAddVertex();
            }
            OSS << "} else {\n";
            {
                INDENT();
                OSS << "bAdded = false;\n";
                OSS << "v = static_cast<Vertex>(std::distance(mEmptyBitset.begin(), iter));\n";
                OSS << "*iter = false;\n";
                outputSetVertex();
            }
            OSS << "}\n";
        } else {
            OSS << "auto v = static_cast<Vertex>(num_vertices());\n";
        }
    }
    ...
    if (!s.isVector()) {
        OSS << "auto* v = &mVertices.back();\n";
    }

    if (s.mReferenceGraph) {
        OSS << "// HierarchyGraph\n";
        OSS << "if (u != null_vertex()) {\n";
        {
            INDENT();
            OSS << "add_link(u, v);\n";
        }
    ...
    for (const auto& map : s.mVertexMaps) {
        if (map.isBimap()) {
            ...
            auto keyName = getTagName(map.mComponentName, scratch);
            std::pmr::string resName(keyName, scratch);
            resName.append("Res");
            if (map.mOptional) {
                OSS << "if (!" << keyName << ".empty()) {\n";
                space.append("    ");
            }
            if (piecewise) {
                OSS << "auto " << resName << " = " << map.mMemberName
                    << ".emplace(\n";
                OSS << "    std::piecewise_construct,\n";
                if (params.contains(map.mComponentName)) {
                    OSS << "    std::forward_as_tuple(" << keyName << "),\n";
                } else {
                    OSS << "    " << keyName << "T,\n";
                }
                OSS << "    std::forward_as_tuple(v));\n";
            } else {
                OSS << "auto " << resName << " = " << map.mMemberName
                    << ".emplace(" << keyName << ", v);\n";
            }
            OSS << "Ensures(" << resName << ".second);\n";
            if (map.mOptional) {
    ...
    if (s.isPolymorphic()) {
        ...
        if (piecewise) {
            int count = 0;
            for (const auto& c : s.mPolymorphic.mConcepts) {
                auto valueName = cpp.getDependentName(c.mValue);
                        ...
                        OSS << "[&](auto&&... args) {\n";
                        {
                            ...
                            auto outputParams = [&]() {
                                if (c.isIntrusive()) {
                                    ...
                                    oss << "\n";
                                    OSS << "    static_cast<" << s.mVertexSizeType
                                        << ">(std::size(" << c.mMemberName << ")) ";
                                }
                            };
                            if (s.isVector()) {
                                if (s.mRecycle) {
                                    ...
                                    {
                                        INDENT();
                                        OSS << "mVariants.emplace_back(" << handleElemType(c, ns) << "{";
                                        outputParams();
                                        oss << "});\n";
                                    }
                                    OSS << "} else {\n";
                                    {
                                        INDENT();
                                        OSS << "mVariants[v] = " << handleElemType(c, ns) << "{";
                                        outputParams();
                                        oss << "};\n";
                                    }
                                    OSS << "}\n";
                                ...
                                if (!c.isIntrusive()) {
                                    OSS << c.mMemberName << ".emplace_back(std::forward<decltype(args)>(args)...);\n";
                                }
                            } else {
                                if (c.isIntrusive()) {
                                    OSS << "v->mVariant.emplace<" << handleElemType(c, ns) << ">(std::forward<decltype(args)>(args)...);\n";
                                } else {
                                    OSS << "v->mVariant.emplace<" << handleElemType(c, ns) << ">(\n";
                                    {
                                        INDENT();
                                        OSS << "makeUnique<" << valueName << ">(get_allocator(), std::forward<decltype(args)>(args)...));\n";
                                    }
                                }
                            }
                        }
                        OSS << "},\n";
                        OSS << "std::forward<TypeT>(valT));\n";
                    }
                }
                OSS << "}";
            }
            oss << " else {\n";
            OSS << "    static_assert(!sizeof(TypeT), \"value not found in graph\");\n";
            OSS << "}\n";
        } else { // no piecewise
            int count = 0;
            for (const auto& c : s.mPolymorphic.mConcepts) {
                ...
                oss << "if constexpr (std::is_same_v<std::remove_cv_t<std::remove_reference_t<Type>>, "
                    << cpp.getDependentName(c.mValue) << ">) {\n";
                {
                    ...
                    if (s.isVector()) {
                        ...
                        if (!c.isIntrusive()) {
                            OSS << c.mMemberName << ".emplace_back(";
                            oss << "std::forward<Type>(val)";
                            oss << ");\n";
                        }
                    } else {
                        ...
                        if (c.isIntrusive()) {
                            ...
                            {
                                INDENT();
                                OSS << "makeUnique<" << valueName << ">(get_allocator(), std::forward<Type>(val)));\n";
                            }
                        }
                    }
                }
            ...
            oss << " else {\n";
            OSS << "    static_assert(!sizeof(Type), \"value not found in graph\");\n";
            OSS << "}\n";
        } // piecewise
    } // PolymorphicGraph
            ...
            for (int id = 0; const auto& c : s.mComponents) {
                ...
                if (s.isVector()) {
                    if (!c.mCounterName.empty()) {
                        ...
                        {
                            INDENT();
                            OSS << "Expects(" << paramName << ".size() <= " << c.mCounterName << ");\n";
                            OSS << "uint32_t tmpK = 0;\n";
                            OSS << "for (const auto& tmpV : " << paramName << ") {\n";
                            {
                                INDENT();
                                OSS << member << ".emplace_back(tmpV);\n";
                                OSS << "++tmpK;\n";
                            }
                            OSS << "}\n";
                            OSS << "for (; tmpK != " << c.mCounterName << "; ++tmpK) {\n";
                            {
                                INDENT();
                                OSS << member << ".emplace_back();\n";
                            }
                            OSS << "}\n";
                            OSS << "Ensures(" << member << ".size() % " << c.mCounterName << " == 0);\n";
                        }
                        OSS << "}\n";
                        continue;
                    }
                    if (piecewise) {
                        if (params.contains(c.mName)) {
                            OSS << member << ".emplace_back(std::move(" << paramName << "));\n";
                        } else {
                            OSS << "std::apply(\n";
                            {
                                INDENT();
                                OSS << "[&](auto&&... args) {\n";
                                OSS << "    " << member << ".emplace_back(std::forward<decltype(args)>(args)...);\n";
                                OSS << "},\n";
                                OSS << "std::forward<" << removeTail(c.mName) << "T>(" << paramName << "T));\n";
                            }
                        }
                    } else {
                        OSS << member << ".emplace_back(";
                        oss << "std::forward<" << removeTail(c.mName) << ">(" << paramName << ")";
                        oss << ");\n";
                    }
                } else {
                    if (piecewise) {
                        OSS << "v->" << c.mMemberName << " = std::move(" << paramName << ");\n";
                    } else {
                        OSS << "v->" << c.mMemberName << " = std::forward<" << removeTail(c.mName) << ">(" << paramName << ");\n";
                    }
                }
                ++id;
            }
        };
        auto outputSetComponent = [&]() {
            for (int id = 0; const auto& c : s.mComponents) {
                const auto componentID = locate(c.mValuePath, g);
                auto bPmr = g.isPmr(componentID);
                const auto& member = c.mMemberName;
                auto paramName = getTagName(c.mName, scratch);
                Expects(s.isVector());
                auto cName = g.getDependentCppName(ns, componentID);
                if (piecewise) {
                    if (params.contains(c.mName)) {
                        if (bPmr) {
                            OSS << member << "[v] = " << cName << "(std::move(" << paramName << "), get_allocator());\n";
                        } else {
                            OSS << member << "[v] = " << cName << "(std::move(" << paramName << "));\n";
                        }
                    } else {
                        OSS << "std::apply(\n";
                        {
                            ...
                            OSS << "[&](auto&&... args) {\n";
                            if (bPmr) {
                                OSS << "    " << member << "[v] = " << cName << "(std::forward<decltype(args)>(args)..., get_allocator());\n";
                            } else {
                                OSS << "    " << member << "[v] = " << cName << "(std::forward<decltype(args)>(args)...);\n";
                            }
                            OSS << "},\n";
                            OSS << "std::forward<" << removeTail(c.mName) << "T>(" << paramName << "T));\n";
                        }
...
std::pmr::string ObjectGraphBuilder::addVertexObjectImpl(bool full, bool piecewise) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
        ...
        if (map.isBimap()) {
            ...
            if (piecewise) {
                OSS << "auto " << resName << " = " << map.mMemberName
                    ...
                    << keyName << ", std::forward_as_tuple(v));\n";
            } else {
                OSS << "auto " << resName << " = " << map.mMemberName
                    << ".emplace(" << keyName << ", v);\n";
            }
...
std::pmr::string ObjectGraphBuilder::addVertex(bool propertyParam, bool piecewise) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
...
std::pmr::string ObjectGraphBuilder::generateAddressableGraph(bool bInline) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
    ...
    OSS << "children(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
    ...
    OSS << "num_children(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
    ...
    OSS << "ownership(" << name << "::Vertex u, " << name << "::Vertex v, " << name << "& g) noexcept {\n";
    {
        INDENT();
        ...
        OSS << "return std::pair{ " << name << "::ownership_descriptor(u, v), hasEdge };\n";
    }
    OSS << "}\n";
    ...
    OSS << "parents(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
    OSS << "    return std::make_pair(\n";
    ...
    oss << "\n";
    OSS << "inline " << name << "::children_size_type\n";
    OSS << "num_parents(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
    OSS << "    return gsl::narrow_cast<" << name << "::children_size_type>(g.parents_list(u).size());\n";
    OSS << "}\n";

    oss << "\n";
    OSS << "inline " << name << "::Vertex\n";
    OSS << "parent(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
    ...
    oss << "\n";
    OSS << "inline bool\n";
    OSS << "ancestor(" << name << "::Vertex u, " << name
        << "::Vertex v, const " << name << "& g) noexcept {\n";
    {
        INDENT();
...
std::pmr::string ObjectGraphBuilder::generateAddLayer(bool piecewise) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
...

    {
        ...
        oss << (count++ == 0 ? "" : ", ") << "class Layer";
        oss << ">\n";
    }

    OSS << "inline " << name << "::layer_descriptor\n";
    OSS << "add_layer(";
    if (piecewise) {
        oss << "Tag, ";
    }
    oss << "Layer&& val, " << name << "&g) {\n";
    {
        INDENT();
        OSS << "auto v = gsl::narrow_cast<" << name << "::layer_descriptor>(g.mLayers.size());\n";
        oss << "\n";

        if (piecewise) {
            int count = 0;
            for (const auto& c : s.mStack.mLayers) {
                if (count++)
                    oss << " else ";
                else
                    OSS;
                ...
                oss << "if constexpr (std::is_same_v<std::remove_cv_t<std::remove_reference_t<Tag>>, "
                    << cpp.getDependentName(c.mTagPath) << ">) {\n";
                {
                    INDENT();
                    OSS << "std::apply(\n";
                    {
                        ...
                        OSS << "[&](auto&&... args) {\n";
                        {
                            ...
                            visit(
                                overload(
                                    [&](Vector_) {
                                        OSS << "g.mLayers.emplace_back("
                                            << layerHandleType(c, ns) << "{";
                                        if (c.isIntrusive()) {
                                            oss << " std::forward<decltype(args)>(args)... ";
                                        } else {
                                            oss << "\n";
                                            OSS << "    gsl::narrow_cast<"
                                                << name << "::layer_descriptor>(g." << c.mMemberName << ".size())\n";
                                            OSS;
                                        }
                                        oss << "});\n";
                                    ...
                                    [&](List_) {
                                        if (!c.isIntrusive()) {
                                            OSS << "g.mLayers.emplace_back("
                                                << layerHandleType(c, ns) << "{ g." << c.mMemberName
                        ...
                        OSS << "},\n";
                        OSS << "std::forward<Layer>(val));\n";
                    }
                }
                OSS << "}";
            }
            oss << " else {\n";
            OSS << "    static_assert(!sizeof(Layer), \"value not found in graph\");\n";
            OSS << "}\n";
        } else { // no piecewise
            int count = 0;
            for (const auto& c : s.mStack.mLayers) {
                ...
                oss << "if constexpr (std::is_same_v<std::remove_cv_t<std::remove_reference_t<Layer>>, " << cpp.getDependentName(c.mGraphPath) << ">) {\n";
                {
                    INDENT();
                    visit(overload(
                              [&](Vector_) {
                                  OSS << "g.mLayers.emplace_back(" << layerHandleType(c, ns) << "{";
                                  if (c.isIntrusive()) {
                                      oss << " std::forward<Layer>(val) ";
                                  } else {
                                      oss << "\n";
                              ...
                              [&](List_) {
                                  if (c.isIntrusive()) {
                                      OSS << "g.mLayers.emplace_back(" << layerHandleType(c, ns) << "{ g." << c.mMemberName
...
std::pmr::string ObjectGraphBuilder::generateLayer(bool bConst) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
    ...
    std::pmr::string valueType(bConst ? "layer_const_value_type" : "layer_value_type", scratch);
    oss << "\n";
    OSS << "[[nodiscard]] inline " << name << "::" << valueType << "\n";
    OSS << "layer(" << name << "::layer_descriptor u, ";
    if (bConst)
        oss << "const ";
    oss << name << "& g) noexcept {\n";
    {
        INDENT();
        OSS << "using layer_descriptor = " << name << "::layer_descriptor;\n";
        OSS << "return visit(\n";
        {
            INDENT();
            OSS << "overload(\n";
            {
                ...
                for (const auto& layer : s.mStack.mLayers) {
                    ...
                    oss << layerHandleType(layer, cn, true) << "& h) {\n";
                    {
                        INDENT();
                        if (layer.isIntrusive()) {
...
std::pmr::string ObjectGraphBuilder::generateGetLayer(bool bConst) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
    ...
    oss << "\n";
    OSS << "template <class Layer>\n";
    if (bConst) {
        OSS << "[[nodiscard]] inline const Layer&\n";
    } else {
        OSS << "[[nodiscard]] inline Layer&\n";
    }
    OSS << "get_layer(" << name << "::layer_descriptor v, ";
    if (bConst)
        oss << "const ";
    oss << name << "& g) noexcept {\n";
    {
        ...
        for (const auto& c : s.mStack.mLayers) {
            ...
            oss << "if constexpr (std::is_same_v<std::remove_cv_t<std::remove_reference_t<Layer>>, "
                << cpp.getDependentName(c.mGraphPath) << ">) {\n";
            {
                INDENT();
                OSS << "auto&& h = std::get<" << layerHandleType(c, cn, true) << ">(g.mLayers[v]);\n";
                if (c.mMemberName.empty()) {
                    OSS << "return h.mValue;\n";
                } else {
                    OSS << "return g." << c.mMemberName << "[h.mValue];\n";
                }
            }
            OSS << "}";
        }
        oss << " else {\n";
        OSS << "    static_assert(!sizeof(Layer), \"Value type is not in GraphStack\");\n";
        OSS << "}";
        oss << "\n";
        //OSS << "return ptr;\n";
    }
...
std::pmr::string ObjectGraphBuilder::generateGetLayerIf(bool bConst) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
    ...
    OSS << "get_layer_if(" << name << "::layer_descriptor v, ";
    if (bConst)
        oss << "const ";
    oss << name << "* pGraph) noexcept {\n";
    {
        ...
        OSS << "using layer_descriptor = " << name << "::layer_descriptor;\n";
        if (bConst) {
            OSS << "const Layer* ptr = nullptr;\n";
        } else {
            OSS << "Layer* ptr = nullptr;\n";
        }
        oss << "\n";
        OSS << "if (!pGraph) {\n";
        OSS << "    return ptr;\n";
        ...
        for (const auto& c : s.mStack.mLayers) {
            ...
            {
                INDENT();
                OSS << "auto* pHandle = std::get_if<"
                    << layerHandleType(c, cn, true) << ">(&g.mLayers[v]);\n";
                OSS << "if (pHandle) {\n";
                if (c.mMemberName.empty()) {
...
std::pmr::string ObjectGraphBuilder::generateGraphFunctions_h() const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
    ...
    if (s.mIncidence) {
        ...
        OSS << "out_edges(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
        OSS << "    return std::make_pair(\n";
        ...
        OSS << "edge(" << name << "::Vertex u, " << name << "::Vertex v, const " << name << "& g) noexcept {\n";
        {
            ...
            if (s.hasEdgeProperty() || s.needEdgeList()) {
                OSS << "return std::pair{ " << name << "::edge_descriptor(u, v, (hasEdge ? &(*iter).get_property() : nullptr)), hasEdge };\n";
            } else {
                OSS << "return std::pair{ " << name << "::edge_descriptor(u, v), hasEdge };\n";
            }
        }
        OSS << "}\n";
        ...
        if (s.mUndirected) {
            ...
            OSS << "in_edges(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
            OSS << "    return std::make_pair(\n";
            ...
            oss << "\n";
            OSS << "inline " << name << "::degree_size_type\n";
            OSS << "in_degree(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
            OSS << "    return out_degree(u, g);\n";
            OSS << "}\n";

            oss << "\n";
            OSS << "inline " << name << "::degree_size_type\n";
            OSS << "degree(" << name << "::Vertex u, const " << name << " & g) noexcept {\n";
            OSS << "    return out_degree(u, g);\n";
            OSS << "}\n";
    ...
    if (s.mAdjacency) {
        ...
        OSS << "adjacent_vertices(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
        {
            INDENT();
    ...
    if (s.mEdgeList) {
        ...
        if (s.mMutableGraphEdge) {
            ...
            // remove edge(s)
            oss << "\n";
            OSS << "inline void remove_edge(" << name << "::Vertex u, "
                << name << "::Vertex v, " << name << "& g) noexcept {\n";
            {
                INDENT();
    ...
    return e.get_target() == u;
            ...
            if (s.mIncidence) {
                ...
                OSS << "inline void remove_out_edge_if(" << name << "::Vertex u, Predicate&& pred, " << name << "& g)";
                ...
                if (s.mBidirectional) {
                    ...
                    oss << "\n";
                    OSS << "template <class Predicate>\n";
                    OSS << "inline void remove_in_edge_if(" << name << "::Vertex v, Predicate&& pred, " << name << "& g)";
                    if (!s.mUndirected && !s.hasEdgeProperty())
                        oss << " noexcept";
                    oss << " {\n";
                    {
    ...
    if (s.hasStack()) {
        oss << "\n";
        OSS << "// GraphStack\n";
        OSS << "[[nodiscard]] inline std::pair<" << name << "::layer_iterator, " << name << "::layer_iterator>\n";
        OSS << "layers(const " << name << "& g) noexcept {\n";
        OSS << "    return std::make_pair(const_cast<" << name << "&>(g).layer_set().begin(), const_cast<" << name << "&>(g).layer_set().end());\n";
        OSS << "}\n";

        oss << "\n";
        OSS << "[[nodiscard]] inline " << name << "::layers_size_type\n";
        OSS << "num_layers(const " << name << "& g) noexcept {\n";
        OSS << "    return gsl::narrow_cast<" << name << "::layers_size_type>(g.mLayers.size());\n";
        OSS << "}\n";

        oss << "\n";
        OSS << "[[nodiscard]] inline " << name << "::layers_size_type\n";
        OSS << "layer_id(" << name << "::layer_descriptor u, const " << name << "& g) noexcept {\n";
        {
            INDENT();
            OSS << "using layer_descriptor = " << name << "::layer_descriptor;\n";
            OSS << "return visit(\n";
            {
                ...
                OSS << "overload(\n";
                {
                    ...
                    for (const auto& layer : s.mStack.mLayers) {
                        ...
                        if (layer.mMemberName.empty()) {
                            OSS << "[](const " << layerHandleType(layer, cn, true) << "&) {\n";
                            OSS << "    return " << name << "::null_layer();\n";
                            OSS << "}";
                        } else {
                            OSS << "[](const " << layerHandleType(layer, cn, true) << "& h) {\n";
                            OSS << "    return h.mValue;\n";
                            OSS << "}";
                        }
                    }
        ...
        copyString(oss, space, generateLayer(false));
        copyString(oss, space, generateLayer(true));

        copyString(oss, space, generateGetLayer(false));
        copyString(oss, space, generateGetLayer(true));

        copyString(oss, space, generateGetLayerIf(false));
        copyString(oss, space, generateGetLayerIf(true));

        {
            oss << "\n";
            OSS << "template <class Layer>\n";
            OSS << "[[nodiscard]] inline bool\n";
            OSS << "holds_layer(" << name << "::layer_descriptor v, ";
            oss << "const " << name << "& g) noexcept {\n";
            {
                ...
                for (const auto& c : s.mStack.mLayers) {
                    ...
                    {
                        INDENT();
                        OSS << "return std::holds_alternative<" << layerHandleType(c, cn, true) << ">(g.mLayers[v]);\n";
                    }
                    OSS << "}";
                }
                oss << " else {\n";
                OSS << "    static_assert(!sizeof(Layer), \"Layer type is not in GraphStack\");\n";
                OSS << "}";
                oss << "\n";
                OSS << "return false;\n";
            }
        ...
        oss << "\n";
        copyString(oss, space, generateAddLayer(false));
        oss << "\n";
        copyString(oss, space, generateAddLayer(true));
    }

    return oss.str();
}

std::pmr::string ObjectGraphBuilder::generateGraphBoostFunctions_h() const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
            ...
            if (s.mNamed && s.mNamedConcept.mComponent && c.mName == s.mNamedConcept.mComponentName) {
                ...
                OSS << "struct property_map<" << name << ", vertex_name_t> {\n";
                {
                    ...
                    if (s.mNamedConcept.mComponentMemberName.empty()) {
                        ...
                        std::pmr::string stringName(scratch);
                        auto stringID = g.getMemberType(componentID, s.mNamedConcept.mComponentMemberName);
                        Expects(stringID != g.null_vertex());
                        stringName = cpp.getDependentName(stringID);
                        OSS << "using const_type = ";
                        copyString(oss, space, vertexComponentMapMemberName(c, true, stringName), true);
                        oss << ";\n";
...
std::pmr::string ObjectGraphBuilder::generateGraphPropertyMaps_h() const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
    ...
    if (s.isVector()) {
        ...
        if (s.mColorMap) {
            oss << "\n";
            OSS << "[[nodiscard]] inline Impl::ColorMap<" << name << "::Vertex>\n";
            OSS << "get(PmrVector<boost::default_color_type>& colors, const " << name << "& g) noexcept {\n";
            OSS << "    return { colors };\n";
            OSS << "}\n";
        }
    }
    ...
    if (s.isPolymorphic()) {
        ...
        if (holds_alternative<Vector_>(s.mVertexListType)) {
            ...
            OSS << "value_id(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
            {
                INDENT();
                OSS << "using Vertex = " << name << "::Vertex;\n";
                OSS << "return visit(\n";
        ...
        OSS << "tag(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
        {
            INDENT();
            OSS << "using Vertex = " << name << "::Vertex;\n";
            OSS << "return visit(\n";
        ...
        auto generateHold = [&](bool bTag) {
            ...
            OSS << "[[nodiscard]] inline bool\n";
            if (bTag) {
                OSS << "holds_tag(" << name << "::Vertex v, ";
            } else {
                OSS << "holds_alternative(" << name << "::Vertex v, ";
            }
            oss << "const " << name << "& g) noexcept {\n";
            {
                INDENT();
                ...
                for (const auto& c : s.mPolymorphic.mConcepts) {
                    ...
                    if (bTag) {
                        ...
                        oss << "if constexpr (std::is_same_v<std::remove_cv_t<std::remove_reference_t<ValueT>>, "
                            << cpp.getDependentName(c.mValue) << ">) {\n";
                    }
                    {
                        INDENT();
                ...
                oss << " else {\n";
                if (bTag) {
                    OSS << "    static_assert(!sizeof(Tag), \"Tag type is not in PolymorphicGraph\");\n";
                } else {
                    OSS << "    static_assert(!sizeof(ValueT), \"Value type is not in PolymorphicGraph\");\n";
                }
                OSS << "}";
                oss << "\n";
                OSS << "return false;\n";
    ...
    if (hasPropertyMap) {
        ...
        OSS << "get(Tag tag, const " << name << "& g, " << name << "::Vertex v) noexcept {\n";
        ...
        if (s.hasVertexProperty() || !s.mComponents.empty()) {
            ...
            OSS << "template <class Tag>\n";
            OSS << "[[nodiscard]] inline decltype(auto)\n";
            OSS << "get(Tag tag, " << name << "& g, " << name << "::Vertex v) noexcept {\n";
            OSS << "    return get(get(tag, g), v);\n";
            OSS << "}\n";
    ...
    if (!s.mVertexMaps.empty()) {
        ...
        for (const auto& map : s.mVertexMaps) {
            oss << "\n";
            OSS << "// UuidGraph\n";
            OSS << "[[nodiscard]] inline " << name << "::Vertex\n";
    ...
    if (s.mAddressable) {
        ...
        OSS << "path_length(" << name << "::Vertex u, const " << name << "& g) noexcept {\n";
        {
            INDENT();
        ...
        OSS << "    std::ptrdiff_t& sz, " << name << "::Vertex u,\n";
        OSS << "    const " << name << "& g) noexcept {\n";
        {
            INDENT();
        ...
        auto generatePath = [&](bool general, bool pmr, bool utf8) {
                ...
                << name << "::null_vertex()) {\n";
            {
                ...
                if (general) {
                    ...
                    OSS << "const auto sz0 = std::ssize(prefix);\n";
                    OSS << "auto sz = sz0;\n";
                    oss << "\n";
    ...
    if (s.mAddressable) {
        ...
        auto generateLocate = [&](bool pmr) {
            oss << "\n";
            OSS << "[[nodiscard]] inline " << name << "::Vertex\n";
            OSS << "locate(" << name << "::Vertex u, " << viewName << " relative, const " << name << "& g";
            if (pmr) {
                oss << ",\n";
                OSS << "    std::pmr::memory_resource* mr";
            }
        ...
        if (s.isPolymorphic()) {
            ...
            if (s.mAddressableConcept.mUtf8) {
                ...
                OSS << "get(std::string_view pt, " << name << "& g) {\n";
            }
            ...
            if (s.mAddressableConcept.mUtf8) {
                ...
                OSS << "get(std::string_view pt, const " << name << "& g) {\n";
            }
            OSS << "    auto v = locate(pt, g);\n";
            OSS << "    if (v == " << name << "::null_vertex()) {\n";
            OSS << "        throw std::out_of_range(\"at " << name << "\");\n";
            ...
            if (s.mAddressableConcept.mUtf8) {
                ...
                OSS << "get_if(std::string_view pt, " << name << "* pGraph) noexcept {\n";
            }
            OSS << "    if (pGraph) {\n";
            ...
            if (s.mAddressableConcept.mUtf8) {
                ...
                OSS << "get_if(std::string_view pt, const " << name << "* pGraph) {\n";
            }
            OSS << "    if (pGraph) {\n";
            OSS << "        auto v = locate(pt, *pGraph);\n";
            OSS << "        if (v != " << name << "::null_vertex()) {\n";
    ...
    if (s.mVertexList && s.mMutableGraphVertex && s.mAddressable) {
        oss << "\n";
        OSS << "// MutableGraph(Vertex)\n";
        OSS << "inline void add_path_impl(" << name << "::Vertex u, "
            << name << "::Vertex v, " << name << "& g) {\n";
        {
            INDENT();
            OSS << "// add to parent\n";
        ...
        if (holds_alternative<Map_>(s.mAddressableConcept.mType)) {
            oss << "\n";
            OSS << "inline void remove_path_impl(" << name << "::Vertex u, " << name << "& g) noexcept {\n";
            {
                INDENT();
    ...
    if (s.mVertexList) {
        auto addDefaultVertex = [&](bool str, bool cstr) {
            ...
            if (s.mAddressable) {
                oss << ", " << name << "::Vertex parentID = " << name << "::null_vertex()";
            }
            oss << ") {\n";
            {
                INDENT();
