...
std::pmr::string ObjectGraphBuilder::addVertex(bool propertyParam, bool piecewise, bool cpp14) const {
    pmr_ostringstream oss(std::ios::out, get_allocator());
    std::pmr::string space(get_allocator());
    const auto& g = *mStruct.mSyntaxGraph;
    const auto& s = *mGraph;
...
std::pmr::string ObjectGraphBuilder::generateGraphSerialization_cpp(bool nvp) const {
    return "";
}
